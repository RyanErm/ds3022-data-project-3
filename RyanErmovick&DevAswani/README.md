...
This visualization is a heatmap that shows the locations of bussess based on their latitude and longitude. The heatmap provides not only a unique visualization of bus routes and sites of congestion, but also a visual representation of Washington D.C. and it's neighboring suburbs. 
...
This visualization takes in user input to compare the average delay for two different routes. Users can then compare routes to see which has a worse or better delay. 
